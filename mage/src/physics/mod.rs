...
impl Collision {
    pub fn started(&self) -> bool {
        matches!(
            self,
            Collision::Started(_, _, _) | Collision::StartedTrigger(_, _)
        )
    }

    pub fn entity_id(&self) -> Entity {
        match self {
            Collision::Started(entity, _, _)
            | Collision::StartedTrigger(entity, _)
    ...
    pub fn user_data(&self) -> u128 {
        match self {
            Collision::Started(_, _, u)
            | Collision::StartedTrigger(_, u)
